...
class _AttendanceScanPageState extends State<AttendanceScanPage> {
  ...
  get_sessionData() async {
    ...
    setState(() {
      ...
      destinations = complist.getString('destination').toString();
    });
    print("customer Id is ${(customerID)}");
    print("company Id is ${(companyID)}");
    print("company Shift is ${(shift)}");
    print("company Destination is ${(destinations)}");
  }
 ...
 /* Future<void> scanQRCode() async {
    try {
      ...
      setState(() {
        ...
        if (partQR.text.isNotEmpty && partQR.text != '-1') {
          if (partQR.text.isNotEmpty) {
            var lengthcode = partQR.text;
            print(lengthcode.length);

            if (lengthcode.length == 1) {
              checkINcode();
            } else {
              // productqrScan();
